/**
 * ==============================================
 * Storyblok Page Seeder - Google Apps Script
 * ==============================================
 *
 * 1. Go to script.google.com > New Project
 * 2. Paste this entire file
 * 3. Set your MGMT_TOKEN below
 * 4. Select function "seedAll" and click Run
...
function seedPage(pageKey, config) {
  ...
  Logger.log("\n" + "=".repeat(50));
  Logger.log("Seeding: " + slug + " (content_type: " + config.content_type + ")");
  Logger.log("=".repeat(50));

  // 1. Find existing story
  var existing = findStory(slug);
  if (!existing) {
    Logger.log("  ERROR: Story '" + slug + "' not found!");
    Logger.log("  Create it first in Storyblok: Content > pages > + Entry > Name: '" + pageKey + "' > Content Type: " + config.content_type);
    return false;
  }

  Logger.log("  Found story: id=" + existing.id + ", name='" + existing.name + "'");

  // 2. Merge fields (only fill empty ones)
  ...
  Logger.log("  Fields: " + updated + " updated, " + skipped + " skipped (already filled)");
  ...
  try {
    ...
    Logger.log("  SUCCESS: Story '" + slug + "' updated and published!");
    return true;
  } catch (e) {
...
function seedAll() {
  ...
  Logger.log("Storyblok Page Content Seeder");
  ...
  for (var i = 0; i < pages.length; i++) {
    var pageKey = pages[i];
    var result = seedPage(pageKey, seedData[pageKey]);
    if (result) {
      success++;
    } else {
      failed++;
  ...
  Logger.log("\n" + "=".repeat(50));
  Logger.log("DONE! " + success + " pages seeded, " + failed + " failed.");
  Logger.log("");
  Logger.log("Naechste Schritte:");
  Logger.log("1. Browser localStorage leeren (sb_* Keys)");
  Logger.log("   oder Inkognito-Tab oeffnen");
  Logger.log("2. Website pruefen ob alles geladen wird");
  Logger.log("3. Wenn alles passt: alten page_content Content Type loeschen");
  Logger.log("=".repeat(50));
}

// ── OPTIONAL: Overwrite-Version (ueberschreibt ALLE Felder) ──────────────

function seedAllOverwrite() {
  if (MGMT_TOKEN === "DEIN_MANAGEMENT_TOKEN_HIER") {
    Logger.log("ERROR: Bitte trage deinen Management Token oben ein!");
    return;
  }

  Logger.log("WARNING: OVERWRITE MODE - alle Felder werden ueberschrieben!");
  Logger.log("");
  ...
  for (var i = 0; i < pages.length; i++) {
    ...
    var mergedContent = {};
    var existingKeys = Object.keys(existing.content || {});
    for (var k = 0; k < existingKeys.length; k++) {
      mergedContent[existingKeys[k]] = existing.content[existingKeys[k]];
    }
    mergedContent.component = config.content_type;

    var fieldKeys = Object.keys(config.fields);
    for (var j = 0; j < fieldKeys.length; j++) {
      mergedContent[fieldKeys[j]] = config.fields[fieldKeys[j]];
    }
    ...
    try {
      ...
      Logger.log("SUCCESS: " + slug + " overwritten!");
    } catch (e) {
      Logger.log("FAILED: " + slug + " - " + e.message);
    }
